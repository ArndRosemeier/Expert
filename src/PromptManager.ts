...
- **WHOLE-SUBTREE GUIDANCE BELONGS IN CONTEXT, NOT IN THE BODY**: The document is a hierarchy of layers (a "DOCUMENT STRUCTURE" note tells you this node's layer and the layers beneath it). On expansion, body text is split at the ===Section=== headers into child nodes, and each of THOSE is split again at the next layer, all the way down to the prose leaves. So body text is scattered down a single path, whereas a conditional context item is inherited by the whole subtree below the node it is attached to. Therefore material that is true for a whole node and all of its descendants (shared background, setting facts, a throughline, recurring tone/style, a constraint) must be a context item, NOT body prose. This applies recursively: it is not only about the top of the outline — when you write a ===section===, keep only what is specific to that section in its body and lift anything global to that ENTIRE section into a context item scoped to it (see the context rules), so the child created from it and everything beneath inherit it. Keep each ===section=== body focused on the beats specific to it.
