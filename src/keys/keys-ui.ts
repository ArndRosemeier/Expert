...
export class KeysUI {
    ...
    private async handleCreateKey(e: Event): Promise<void> {
        ...
        try {
            ...
            // Save to storage
            const keyId = await KeyStorage.saveKey(result.key, result.data, nickname || undefined);
            
            // Reload stored keys and update display
            await this.loadStoredKeys();
            await this.updateKeysList();
            await this.updateStorageStats();
            this.showSuccess(`Key created and saved successfully!${nickname ? ` (${nickname})` : ''}`);
    ...
    private generateRandomString(): void {
        const randomString = KeyManager.generateRandomString(12);
    ...
    private setPresetExpiry(): void {
        ...
        const localDateTime = weekFromNow.toISOString().slice(0, 16);
        (document.getElementById('expiration-date') as HTMLInputElement).value = localDateTime;
    }
    ...
    private async validateKey(): Promise<void> {
        ...
        try {
            ...
            const stats = await KeyManager.getKeyStats(key, password);
            ...
            if (validation.valid && validation.data) {
                resultDiv.innerHTML = `
                    <div class="validation-success">
                        ...
                        <div class="key-details">
                            <p><strong>Custom String:</strong> "${validation.data.customString}"</p>
                            ...
                            <p><strong>Time Remaining:</strong> ${this.getTimeRemaining(validation.data.expirationDate)}</p>
                            <p><strong>Key Length:</strong> ${stats.length} characters</p>
                        </div>
                    </div>
                `;
            } else {
                resultDiv.innerHTML = `
                    <div class="validation-error">
                        <h3>❌ Invalid Key</h3>
                        <p><strong>Reason:</strong> ${validation.reason || 'Unknown error'}</p>
                        ${validation.data ? `
                            <div class="expired-details">
                                <p><strong>Was Valid Until:</strong> ${validation.data.expirationDate.toLocaleString()}</p>
                                <p><strong>Custom String:</strong> "${validation.data.customString}"</p>
                            </div>
                        ` : ''}
                    </div>
    ...
    private async updateKeysList(): Promise<void> {
        ...
        // Build HTML for each key (checking status with current time)
    ...
    private attachKeyListEventHandlers(): void {
        ...
        // Add event delegation for copy and delete buttons
        newKeysList.addEventListener('click', async (e) => {
            ...
            if (target.classList.contains('copy-btn')) {
                ...
                if (key) {
                    try {
                        // Fix clipboard API issue - use fallback for all browsers
