...
${e}`;try{let i=null;const a=e.match(/```json\s*(\{[\s\S]*?\})\s*```/);if(a&&a[1])i=a[1];else{const s=e.indexOf("{"),u=e.lastIndexOf("}");s!==-1&&u!==-1&&u>s&&(i=e.substring(s,u+1))}if(i){const s=JSON.parse(i);typeof s.score=="number"&&(o=s.score),typeof s.justification=="string"&&(n=s.justification)}}catch(i){console.warn("Could not parse rating response as JSON, using fallback.",{response:e,error:i})}return{criterion:t.name,goal:t.goal,score:o,justification:n}}createEditorPrompt(e,t){return this.prompts.editor.replace("{{response}}",e).replace("{{ratings}}",JSON.stringify(t,null,2))}}const j="expert_app_settings_profiles",D="expert_app_last_used_profile";function ve(r){return typeof r!="object"||r===null?!1:Object.values(r).every(e=>typeof e=="object"&&e!==null&&"prompt"in e&&typeof e.prompt=="string"&&"criteria"in e&&Array.isArray(e.criteria)&&"maxIterations"in e&&typeof e.maxIterations=="number"&&"selectedModels"in e&&typeof e.selectedModels=="object"&&e.selectedModels!==null)}class xe{constructor(){f(this,"profiles",{});f(this,"lastUsedProfileName",null);this.loadProfiles(),this.lastUsedProfileName=localStorage.getItem(D)}loadProfiles(){const e=localStorage.getItem(j);if(e)try{const t=JSON.parse(e);ve(t)?this.profiles=t:(console.warn("Invalid settings profiles found in localStorage. Ignoring."),this.profiles={})}catch(t){console.error("Failed to parse settings profiles from localStorage",t),this.profiles={}}}getProfileNames(){return Object.keys(this.profiles)}getProfile(e){return this.profiles[e]}saveProfile(e,t){if(!e)throw new Error("Profile name cannot be empty.");this.profiles[e]=t,localStorage.setItem(j,JSON.stringify(this.profiles)),this.setLastUsedProfile(e)}deleteProfile(e){delete this.profiles[e],localStorage.setItem(j,JSON.stringify(this.profiles)),this.lastUsedProfileName===e&&(localStorage.removeItem(D),this.lastUsedProfileName=null)}getLastUsedProfile(){if(this.lastUsedProfileName)return this.getProfile(this.lastUsedProfileName);const e=this.getProfileNames();return e.length>0?this.getProfile(e[0]):void 0}getLastUsedProfileName(){if(this.lastUsedProfileName&&this.profiles[this.lastUsedProfileName])return this.lastUsedProfileName;const e=this.getProfileNames();return e.length>0?e[0]:null}setLastUsedProfile(e){this.lastUsedProfileName=e,localStorage.setItem(D,e)}}function l(r){const e=document.getElementById(r);if(!e)throw new Error(`Could not find element with id: ${r}`);return e}const N=l("initial-setup"),O=l("main-app"),ee=l("configure-models-btn"),te=l("configure-prompts-btn"),k=l("modal-container"),re=l("modal-content"),M=l("prompt-modal-container"),oe=l("prompt-modal-content");if(!N||!O||!ee||!k||!re||!te||!M||!oe)throw new Error("Could not find required DOM elements");let $=null,x=null,V={},P=null,v=null,F={...T},R=[],A=0;const ne=[{name:"Clarity & Conciseness. The writing is direct, easy to understand, and avoids unnecessary words or filler phrases.",goal:8},{name:"Natural & Authentic Tone. The language sounds human and authentic. It avoids being overly formal, academic, or robotic.",goal:9},{name:"Engaging Flow. The text is interesting and holds the reader's attention. Sentences and paragraphs transition smoothly.",goal:8},{name:"Varied Sentence Structure. The length and structure of sentences are varied to create a pleasing rhythm, avoiding monotony.",goal:7},{name:"Subtlety (Show, Don't Tell). The writing implies emotions and ideas through description and action rather than stating them directly. It avoids being on-the-nose.",goal:8},{name:"Avoids AI Clichés. The text avoids common AI phrases like 'In conclusion,' 'It's important to note,' 'delve into,' or 'tapestry of...'",goal:9},{name:"Understated Language. The prose avoids overly dramatic, sensational, or grandiose language. The tone is measured and appropriate.",goal:9},{name:"Specificity & Concrete Detail. The writing uses specific, concrete details and examples rather than vague generalities.",goal:8},{name:"Original Phrasing. The text avoids common idioms and clichés, opting for more original ways to express ideas.",goal:7},{name:"Human-like Naming. When applicable, any names for people, places, or concepts are creative and feel natural. Avoid common AI-generated names like 'Elara' or 'Lyra.'",goal:8}];function we(){k&&(k.style.display="flex")}function z(){k&&(k.style.display="none")}function Se(){M&&(M.style.display="flex")}function ie(){M&&(M.style.display="none")}function H(r){console.log("Reconfiguring core services with models:",r),V=r;const e=x==null?void 0:x.getApiKey();e&&($=new X(e,V),P=new Z($,F),console.log("OpenRouter client and Orchestrator configured."))}function Ee(r){H(r),z(),N.style.display!=="none"&&(se(),N.style.display="none",O.style.display="block")}function Ce(r){F=r,$&&(P=new Z($,F)),ie()}function se(){var r,e,t,o,n,i,a,s,u,m,p;O.innerHTML=`
    ...
    `;const t=e.querySelector("textarea"),o=e.querySelector('input[type="number"]');return r&&(t.value=_(r.name),t.dataset.fullText=r.name,o.value=String(r.goal)),t.addEventListener("focus",()=>{t.value=t.dataset.fullText||"",Q.call(t)}),t.addEventListener("blur",()=>{const i=t.value.trim();i?(t.dataset.fullText=i,t.value=_(i)):(delete t.dataset.fullText,t.value=""),t.style.height="auto"}),t.addEventListener("input",Q),t.addEventListener("keydown",i=>{i.key==="Enter"&&!i.shiftKey&&(i.preventDefault(),i.target.blur())}),(n=e.querySelector(".remove-criterion-btn"))==null||n.addEventListener("click",()=>e.remove()),e}function Te(){const r=I(),e=new Map;r.forEach(o=>e.set(o.name,o)),ne.forEach(o=>e.set(o.name,o));const t=Array.from(e.values());K(t),alert("Default criteria loaded and merged!")}async function Re(){const r=I();if(r.length===0){alert("No criteria to copy.");return}try{await navigator.clipboard.writeText(JSON.stringify(r,null,2)),alert("Criteria copied to clipboard!")}catch(e){console.error("Failed to copy criteria: ",e),alert("Failed to copy criteria to clipboard.")}}async function Ae(){try{const r=await navigator.clipboard.readText(),e=JSON.parse(r);if(!Me(e))throw new Error("Clipboard does not contain a valid criteria array.");const t=I(),o=new Map;t.forEach(i=>o.set(i.name,i)),e.forEach(i=>o.set(i.name,i));const n=Array.from(o.values());K(n),alert("Criteria pasted and merged!")}catch(r){console.error("Failed to paste criteria: ",r),alert("Failed to paste criteria. Please make sure the clipboard contains a valid JSON array of criteria.")}}function ce(r){l("prompt").value=r.prompt,l("max-iterations").value=String(r.maxIterations),K(r.criteria)}function pe(){if(!v)return;const r=l("settings-profile-select"),e=v.getLastUsedProfileName();r.innerHTML="";const t=v.getProfileNames();if(t.length===0){r.innerHTML="<option disabled>No profiles saved</option>";return}t.forEach(o=>{const n=document.createElement("option");n.value=o,n.textContent=o,o===e&&(n.selected=!0),r.appendChild(n)})}function Ne(){if(!v||!x)return;const r=l("settings-profile-name");let e=r.value.trim();if(!e){const i=l("settings-profile-select").value;if(i&&confirm(`No new profile name entered. Do you want to overwrite the selected profile "${i}"?`))e=i;else return}const t={prompt:l("prompt").value,maxIterations:parseInt(l("max-iterations").value,10),criteria:I(),selectedModels:x.getSelectedModels()};v.saveProfile(e,t),r.value="",pe();const o=l("settings-profile-select");o.value=e}function Oe(r){if(!v||!x)return;const e=r.target;if(!e)return;const t=e.value,o=v.getProfile(t);o&&(ce(o),x.setSelectedModels(o.selectedModels),H(o.selectedModels),v.setLastUsedProfile(t))}function Be(){if(!v)return;const e=l("settings-profile-select").value;e&&confirm(`Are you sure you want to delete the "${e}" profile?`)&&(v.deleteProfile(e),ue())}function ue(){if(!v)return;const r=v.getLastUsedProfile();r&&ce(r),pe()}ee.addEventListener("click",we);te.addEventListener("click",Se);k.addEventListener("click",r=>{r.target===k&&z()});M.addEventListener("click",r=>{r.target===M&&ie()});x=new ye(Ee,z);x.render(re);new be(oe,Ce);v=new xe;const $e=x.getApiKey(),Ke=x.areAllModelsSelected();$e&&Ke?(console.log("Models already configured, showing main app."),H(x.getSelectedModels()),se(),N.style.display="none",O.style.display="block"):(O.style.display="none",N.style.display="block");console.log("Application initialized.");
